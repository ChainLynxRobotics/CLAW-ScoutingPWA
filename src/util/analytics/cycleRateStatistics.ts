 ...
 */
export function describeCycleRateQuantitativeObjects<T>(paths: Leaves<T>[], positive: T[][], negative?: T[][]) {
    return subtractQuantitativeData(
        addQuantitativeData(
            ...positive.map(sampleObj => describeQuantitativeData(sampleObj.map(o => describeCycleRateQuantitativeData(paths, o)).filter(o => o !== null) as number[]))
        ),
        negative && addQuantitativeData(
            ...negative.map(sampleObj => describeQuantitativeData(sampleObj.map(o => describeCycleRateQuantitativeData(paths, o)).filter(o => o !== null) as number[]))
        )
    )
}

export function describeCycleRateQuantitativeData<T>(paths: Leaves<T>[], object: T): number|null {
    const sum = sumAtPaths(object, paths);
    if (sum === 0) return null;
    return matchLengthSeconds / sum;
}